...
public class WorldComponent extends JComponent {
	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 0; i <= World.SIZE; i++) {
			Line2D hline = new Line2D.Double(0, i * GRID_SIZE, 600, i
					* GRID_SIZE);
			Line2D vline = new Line2D.Double(i * GRID_SIZE, 0, i * GRID_SIZE,
					600);
			g2d.draw(hline);
			g2d.draw(vline);
		}
		...
		for (Point fuel : world.getAvailableFuel()) {
			int x = fuel.x * GRID_SIZE + GRID_SIZE / 2 - fuelImage.getWidth()
					/ 2;
			int y = fuel.y * GRID_SIZE + GRID_SIZE / 2 - fuelImage.getHeight()
					/ 2;
	...
	private class AnimationTask extends TimerTask {
		...
		public void run() {
			...
			if (r1dead || r2dead) {
				timer.cancel();
				timer = null;
				String msg = (r1dead && r2dead) ? "Both robots"
						: r1dead ? "Robot 1 (red)" : "Robot 2 (blue)";
